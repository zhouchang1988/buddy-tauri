...
const en = {
  ...
  'settings.prompts.desc': 'Extra instructions appended to the end of the system prompt sent to actors every round. The shared prompt goes to both actors; role prompts go only to the actor playing that role. Leave empty to append nothing.',
  'settings.prompts.sharedLabel': 'Shared (both actors)',
...
const zhCN: typeof en = {
  ...
  'settings.prompts.desc': '追加在每轮系统提示词末尾的额外指令。通用提示词两个 actor 都会收到；角色提示词只发给承担该角色的 actor。留空则不追加。',
  'settings.prompts.sharedLabel': '通用（两个 actor 都会收到）',
...
const zhTW: typeof en = {
  ...
  'settings.prompts.desc': '追加在每輪系統提示詞末尾的額外指令。通用提示詞兩個 actor 都會收到；角色提示詞只發給承擔該角色的 actor。留空則不追加。',
  'settings.prompts.sharedLabel': '通用（兩個 actor 都會收到）',
