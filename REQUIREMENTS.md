...
> **编者注（Tauri 版）**：本章描述的是 Electron 版的实现架构。Tauri 版中，「Electron 主进程」由 Rust 后端（`src-tauri/src/buddy/`，`BuddyCoreService` = `BuddyStore` + `BuddyRunner` + `BuddyEventBus` + `QueueCoordinator`）承担，IPC channel `buddy:xxx` 对应 Tauri command `buddy_xxx`，node-pty 对应 `portable-pty` crate，Zod 读时校验对应 `schemas.rs` 的 serde 校验。产品需求与数据 Schema 不变。
...
> **编者注（Tauri 版）**：本章的 `buddy:xxx` IPC channel 在 Tauri 版中实现为 snake_case command `buddy_xxx`（注册于 `src-tauri/src/lib.rs`，前端经 `src/lib/tauri-bridge.ts` 的 `invoke` 调用）；推送事件 `buddy:event` 由 `BuddyEventBus`（tokio broadcast）经 `app.emit` 发出，事件名不变。
...
> **编者注（Tauri 版）**：Tauri 版的等效约束为：渲染进程无 Node API，所有系统能力经 `src-tauri/capabilities/default.json` 最小授权；CSP 在 `src-tauri/tauri.conf.json` 中收紧为 `default-src 'self'`；前端仅通过 `src/lib/tauri-bridge.ts` 访问后端 command。
