...
pub struct BuildActorPromptInput {
    ...
    /// Append the Cursor turn-lifecycle section (native Cursor runs with
    /// `--single-turn`).
    pub cursor_single_turn: Option<bool>,
    /// Append the Buddy task-services section (managed background services).
    pub managed_services: Option<bool>,
}

pub const CURSOR_SINGLE_TURN_INSTRUCTIONS: &str = r#"## Cursor turn lifecycle
...
pub const MANAGED_SERVICE_INSTRUCTIONS: &str = r#"## Task services

For a service needed across turns, invoke this short command in the desired working directory:
"$BUDDY_SERVICE_CLI" start NAME -- COMMAND ARGUMENTS
...
pub fn build_actor_prompt(input: &BuildActorPromptInput) -> String {
    ...
    if input.cursor_single_turn.unwrap_or(false) {
        parts.push(String::new());
        parts.push(CURSOR_SINGLE_TURN_INSTRUCTIONS.to_string());
    }
    if input.managed_services.unwrap_or(false) {
        parts.push(String::new());
        parts.push(MANAGED_SERVICE_INSTRUCTIONS.to_string());
    }
...
mod tests {
    ...
    #[test]
    fn explains_cursor_turn_lifecycle_and_task_services_only_when_flagged() {
        for state in [
            None,
            Some(serde_json::json!({ "cursor_session_id": "existing-session" })),
        ] {
            let input = BuildActorPromptInput {
                actor: "cursor".to_string(),
                round: 2,
                task_text: "Run a worker".to_string(),
                state,
                ..base_input("cursor")
            };
            let prompt = build_actor_prompt(&BuildActorPromptInput {
                cursor_single_turn: Some(true),
                managed_services: Some(true),
                ..input.clone()
            });
            assert!(prompt.contains("## Cursor turn lifecycle"));
            assert!(prompt.contains("Wait for finite work"));
            assert!(prompt.contains("--single-turn"));
            assert!(prompt.contains("BUDDY_SERVICE_CLI"));
            assert!(prompt.contains("PID, log path and stop_command"));
            assert!(prompt.contains("Only when the user explicitly asks"));

            let plain = build_actor_prompt(&input);
            assert!(!plain.contains("## Cursor turn lifecycle"));
            assert!(!plain.contains("## Task services"));
        }
    }
}
